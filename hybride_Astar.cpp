...
} // this is a strategy of O(1) to calculate heuristic
...
int HBF::theta_to_stack_number(double theta) {
    ...
    double new_theta = fmod((theta + 2 * M_PI), (2 * M_PI)); // not "fmod(theta, 2*M_PI)" because theta might be negative
...
vector<HBF::maze_s> HBF::expand(HBF::maze_s& state, vector<int>& goal) {
    ...
    for (double delta_i = -35; delta_i < 40; delta_i += 1) {  // discretization strategy
        double time_step = 1;
        double delta = M_PI / 180.0 * delta_i;
        double omega = SPEED / LENGTH * tan(delta);
        double theta2 = theta + omega * time_step;
        if (theta2 < 0) {
            theta2 += 2 * M_PI;
        }

        double x2 = x + SPEED * time_step * cos(theta); //discretization strategy
        double y2 = y + SPEED * time_step * sin(theta);
        HBF::maze_s state2;
...
HBF::maze_path HBF::search(vector< vector<int> >& grid, vector<double>& start,
    ...
    while (!opened.empty()) {
        ...
        //??? use tree or graph: the list needs to update the value if a smaller value is found
        // solution: use priority_queue to store each point, use harsh table to store the address of point in the priority_queue

        int x = current.x;    
        int y = current.y;
        auto theta = current.theta;        

        if (idx(x) == goal[0] && idx(y) == goal[1] && fmod(theta, 2 * M_PI) <= 0.1) {
            std::cout << "found path to goal in " << total_closed << " expansions"
                << std::endl;
            std::cout << "theta is " << fmod(theta, 2 * M_PI) << std::endl;
            maze_path path;
        ...
        for (size_t i = 0; i < next_state.size(); ++i) {
            ...
            if ((x2 < 0 || x2 >= grid.size()) || (y2 < 0 || y2 >= grid[0].size())) { //using grid is a simplification stragety
                // invalid cell
                continue;
            }
            ...
            if (closed[stack2][idx(x2)][idx(y2)] == 0 && grid[idx(x2)][idx(y2)] == 0) {
                ...
                closed[stack2][idx(x2)][idx(y2)] = 1; //approximation strategy, may result in closing of a grid cell prematurely
